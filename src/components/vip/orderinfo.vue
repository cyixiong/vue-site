                    ...
                    <div class="form-box accept-box form-box1">
                      <dl class="head form-group">
                        <dd>
                          ...
                          <!-- 当订单的状态为3 -->
                          <a href="javascript:;" v-if="info.orderinfo.status==3" class="btn-pay" @click="complate">签收</a>
                          <!---->
                        </dd>
                      </dl>
                      <dl class="form-group">
                        <dt>订单状态：</dt>
                        <dd>{{info.orderinfo.statusName}}</dd>
                      </dl>
                      <dl class="form-group">
                        <dt>快递单号：</dt>
                        <dd>
                            {{info.orderinfo.expressTitle}}
                        </dd>
                      </dl>
                      <dl class="form-group">
                        <dt>支付方式：</dt>
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td colspan="7" align="right">
                              <p>商品金额：
                                <b class="red">￥{{orderTotalAmout}}</b>&nbsp;&nbsp;+&nbsp;&nbsp;运费：
                                <b class="red">￥{{info.orderinfo.express_fee}}</b>
                              </p>
                              <p style="font-size: 22px;">应付总金额：
                                <b class="red">￥{{orderTotalAmout + info.orderinfo.express_fee}}</b>
                              </p>
                            </td>
                          </tr>
                        </tbody>
...
<script>
  export default {
    ...
    methods: {
      complate(){
        this.$ajax.get('/site/validate/order/complate/'+this.orderid)
        .then(res=>{
      ...
      getorderinfo(){
        ...
        .then(res=>{
          ...
          //遍历info中goodslist中的每个商品的真正价格*商品的数量
          this.info.goodslist.forEach((item)=>{
            this.orderTotalAmout += (item.real_price * item.quantity);
          })
        })
      }
    }
